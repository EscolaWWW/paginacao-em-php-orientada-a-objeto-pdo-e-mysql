...
class Funcionario{
	...
	public function sairFuncionario(){
		session_destroy();
		header('location: http://localhost/paginacao-v1');
	}
			...
			$html .= '<div class="panel-footer altr">';
				...
				if($pagina > 1){
                	...
                	$html .= '<ul class="pagination pagination-sm">';
                    	$html .= '<li><a href="/paginacao-v1/admin/funcionario/?pag='.($pagina - 1).'" title="Voltar" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
					$html .= '</ul>';
				}
				if($pagina < $ultima_pag){
					//BT PROXIMO
                	$html .= '<ul class="pagination pagination-sm mgn"> '; 
                    	$html .= '<li><a href="/paginacao-v1/admin/funcionario/?pag='.($pagina + 1).'" title="Próximo" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
					$html .= '</ul>';
				}
				$html .= '</nav>';
        	$html .= '</div>';
